...
export async function GlobalDashboardSidebar() {
  ...
  const initialCollapsed = cookies().get("sidebar_collapsed")?.value === "1";
  ...
  return (
    <SidebarShell initialCollapsed={initialCollapsed}>
      <div className="mb-6">
        <p className="font-display font-semibold text-ink">{session.nama}</p>
        <p className="mt-1 text-xs font-mono text-core">{session.nim ?? "—"}</p>
